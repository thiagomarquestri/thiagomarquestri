# 👋 Hi, I'm Thiago Marques

🎓 **Análise e Desenvolvimento de Sistemas** – 5º semestre  
🏫 **PUC Minas – Campus Contagem**  
📊 **Data Focus | SQL | Analytics | BI**  
🎯 **Seeking Data Internship**

---

## 🇧🇷 Sobre mim

Sou estudante de **Análise e Desenvolvimento de Sistemas** na **PUC Minas Contagem**, atualmente no **5º semestre**, com foco total na **área de Dados**.

Tenho afinidade com **análise de dados, SQL avançado, Excel e Power BI**, sempre buscando transformar dados em **informações úteis para tomada de decisão**.  
Atualmente estou **aprimorando SQL avançado** e iniciando meus estudos em **Python aplicado a dados**.

---

## 🇺🇸 About Me

I'm a **Systems Analysis and Development** student at **PUC Minas – Contagem**, currently in my **5th semester**, focused on the **Data field**.

I have strong interest in **data analysis, advanced SQL, Excel, and Power BI**, aiming to turn raw data into **clear insights for decision-making**.  
Currently improving **advanced SQL** skills and starting studies in **Python for data analysis**.

---

## 🎯 Career Objective

> Work as a **Data Intern**, applying **SQL, Excel, Power BI**, and soon **Python**, to analyze data, generate insights, and support business decisions.

---

## 🛠️ Technologies & Tools

### 📌 Data Focus
<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/mysql/mysql-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/microsoftsqlserver/microsoftsqlserver-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/excel/excel-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/powerbi/powerbi-original.svg" width="40"/>
</p>

---

### 📌 Development & Version Control
<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/csharp/csharp-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/git/git-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/github/github-original.svg" width="40"/>
</p>

---

### 📚 Currently Learning
<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/python/python-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/pandas/pandas-original.svg" width="40"/>
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon@latest/icons/numpy/numpy-original.svg" width="40"/>
</p>

---

## 📊 Skills Summary

- ✅ **SQL:** Intermediate / Advanced  
- ✅ **Excel:** Advanced  
- ⚙️ **Power BI:** Intermediate  
- ⚙️ **Git & GitHub:** Intermediate  
- ⚙️ **C#:** Basic / Intermediate  
- 📚 **Python (Data):** Learning

---

## 📂 Academic Projects

📌 **Projects developed during college**, including:
- Database modeling  
- SQL queries with joins, filters, and aggregations  
- Data organization and analysis  
- Systems developed in C#  

🔧 Repositories are being organized and documented.

---

## 💡 Next Data Projects (Planned)

- 📊 Sales Analysis using **SQL + Power BI**  
- 📈 KPI Dashboard for business decision support  
- 🐍 Data analysis using **Python + CSV/Excel datasets**

---

## 📫 Contact

<p align="left">
  <a href="mailto:thiiagomarqueesbr@gmail.com">
    <img src="https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>

  <a href="https://www.linkedin.com/in/thiiagomarques/" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
</p>


---

## ⚡ Fun Fact

I have professional experience outside the technology field, which helped me develop **organization, responsibility, teamwork, and business vision**.
